        ...
        if event["name"] == "click":
            ...
            if id=="buttonPay":
                ...
                if not is_address(recipient):
                    modal_dialog('Error','Invalid Unobtainum address')
                    continue
...
def receive_loop():
    ...
    while out is None:
        ...
        if not event:
            ...
            amount = modal_input('Amount', 'Amount you want to receive (in UNO). ', format_satoshis(receive_amount) if receive_amount else None, "numberDecimal")
